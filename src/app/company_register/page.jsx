...
const CompanyRegister = () => {
  ...
  async function handleRegisterClick() {
    const result = await fetch(
      `http://localhost:3000/api/company?email=${email}&password=${password}&name=${companyName}&site=${site}`,
      {
        method: "POST",
        headers: {
        ...
        <div className="rounded-2xl bg-accent2 shadow-2xl flex flex-row w-2/3 max-w-4xl">
          <div className="w-2/5 rounded-tl-2xl rounded-bl-2xl bg-accent1 text-bg py-36 px-12">
            ...
            <p className="text-lg text-bg">
              Want to add new game to the party, or just wanting to see the
              results? Login and join us back up!
            </p>
            <button
